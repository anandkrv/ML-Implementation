...
trainin = args.traininput
validin = args.validinput
trainout = args.trainout
validout = args.validout
metricf = args.metricsout
numepoch = int(args.numepoch)
hiddenu = int(args.hiddenu)
initflag = int(args.initflag)
lr = float(args.lr)
...
lbll, attl, lbdic = csvtomatrix(trainin, ohd)
lbll2, attl2, lbdic2 = csvtomatrix(validin,ohd)
model = init_model(hiddenu, 128, 10, initflag)


cet = []
cev = []
for ac in range(numepoch):
    suma = 0
    sumv = 0
    for i in range(len(lbll)):    
        y_pred, J, b, z, a, x = forwardpass(model, attl[i], lbll[i])
        galpha, gbeta, gb, gz, ga = backwardpass(model, y_pred, lbll[i], z, b, lr, i, lbll, attl[i])
        qqq=1
        model[0] = model[0] - lr*galpha.copy()
        model[1] = model[1] - lr*gbeta.copy()
    ...
    cet.append(suma/len(lbll))
    cev.append(sumv/len(lbll2))
