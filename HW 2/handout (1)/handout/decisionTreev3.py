...
def subset_rows(datacopy, val, num):
...
def list_dict(data):
    ...
    for i in range(len(data[0])):
        a_2.append(1)
    return [a_n,a_0,a_1,a_2]
    

print(list_dict(csvtomatrix("education_train.csv")))
...
def info_gain2(data, att_list):
    ...
    for i in range(len(data[0])-1):
        if att_list[3][i] != 0:
            b1.append(entropy(data)-cond_ent(data,i+1))
            b2.append(i)
        else:
            b1.append(None)
            b2.append(None)
...
print(csvtomatrix("education_train.csv")[0])
print(act_index(csvtomatrix("education_train.csv"),"F"))
...
#        print(root.att_name)
        ...
        subset0 = subset_chop(subset_rows(data,indexcheck,0),indexcheck)
        subset1 = subset_chop(subset_rows(data,indexcheck,1),indexcheck)
...
recur(root,0,11,csvtomatrix("education_train.csv"),[])
...
trainerr = err_output_write(root,csvtomatrix("education_train.csv"), "check.txt")
#print(trainerr)

testerr = err_output_write(root,csvtomatrix("education_test.csv"), "check1.txt")
#
...
write_errfile(trainerr, testerr, "check2.txt")
